...
extension ValidatorX on BuildContext {
  ValidationBuilder get validator {
    ...
    return ValidationBuilder(optional: false, requiredMessage: locale.required(), locale: locale);
  }
}
...
extension DateTimeFormatExtension on DateTime {
  /// Converts DateTime to format: 2024-7-23 11:23 AM
  String toCustomFormat() {
    ...
    return '$year-$month-$day $hour12:${minute.toString().padLeft(2, '0')} $period';
  }

  /// Alternative method with more readable name
  String toYearMonthDayTimeFormat() {
    return toCustomFormat();
  }

  /// Method that allows custom separator for date parts
  String toCustomFormatWithSeparator({String dateSeparator = '-'}) {
    int hour12 = hour == 0 ? 12 : (hour > 12 ? hour - 12 : hour);
    String period = hour < 12 ? 'AM' : 'PM';

    return '$year$dateSeparator$month$dateSeparator$day $hour12:${minute.toString().padLeft(2, '0')} $period';
  }
}
