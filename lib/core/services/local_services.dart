...
Future<void> fetchPage<T>({
  ...
  try {
    ...
    if (isLastPage) {
      ...
      pagingController.appendPage(items, nextPageKey as int?);
    }
  } catch (error) {
...
Future<CroppedFile?> cropImage(BuildContext context) async {
  ...
  return await ImageCropper().cropImage(
    ...
    uiSettings: [
      ...
      IOSUiSettings(title: context.l10n.crop),
    ],
  );
}
...
String splitMoney(double value) {
  return value
      .toStringAsFixed(0)
      .splitMapJoin(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), onMatch: (m) => '${m[1]},');
}
